...
<p>
        ...
        up, type `pip3 install cython requests pyminizip ruamel.std.encryptedzip Pycryptodomex psutil`{:.bash}. If you get an error here saying `pip3 cannot be found`
        then you will need to find where <i>pip3.exe</i> is located: `where pip3`{:.bash} and use the `cd`{:.bash} to get to it. Run the same command again when you are
    ...
    <p>
        ...
        update; sudo apt install python3-dev python3-tk -y`{:.bash}, replacing <i>apt</i> with your distribution's package manager. This should add everything to your PATH automatically. Once installed completely, you will also have Tk and Tcl which is 
        needed for a GUI. You will then need to <i>cd</i> into the extracted archive, and issue the command `pip3 install cython requests pyminizip ruamel.std.encryptedzip Pycryptodomex psutil`{:.bash}. This will
        ...
        sudo apt install curl -y; curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py; python3.9 get-pip.py`{:.bash} When completed, you can then double-click the .py files in the /UNIX folder of the archive you extracted earlier 
